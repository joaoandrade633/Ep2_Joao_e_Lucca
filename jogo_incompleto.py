...
def transforma_base(questoes):
    ...
    for questao in questoes:
        if questao['nivel'] not in dict_nivel:
            dict_nivel[questao['nivel']] = [questao]
        else:
            dict_nivel[questao['nivel']] += [questao]
    return dict_nivel
...
def sorteia_questao(dict_nivel, nivel):
    questoes_nivel = questoes.get(nivel, [])
    if not questoes_nivel:
        return None
    return random.choice(questoes_nivel)

def sorteia_questao_inedita(dict_nivel, nivel, questoes_sorteadas):
    questoes_do_nivel = dict_nivel.get(nivel, [])
...
def jogo():
    dict_nivel = transforma_base(questoes)
    questoes_sorteadas = []
    dinheiro_atual = 0
    pulos = 0
    ajudas = 0
    lista_respostas = ["A", "B", "C", "D"]
    lista_respostas_2 = ["ajuda", "pula", "parar"]
    ...
    id_questao = 0
    while True:
        nivel_escolhido = random.choice(list(dict_nivel.keys()))
        questao = sorteia_questao_inedita(dict_nivel, nivel_escolhido, questoes_sorteadas)
        ...
        if resposta == lista_respostas_2[2]:
            print("Fim do jogo!")
            print('dinheiro atual:',dinheiro_atual)
            break

        elif resposta == lista_respostas_2[1] and pulos < 3:
            pulos += 1
            if len(questoes_sorteadas) < len(questoes):
                questoes_sorteadas.append(questao)
                print("Voce pulou para a próxima pergunta.")
                print('dinheiro atual:',dinheiro_atual)
        elif resposta == lista_respostas_2[1] and pulos >= 3:
            print("vc não tem mais pulos")
            print('dinheiro atual:',dinheiro_atual)

        elif resposta == lista_respostas_2[0] and ajudas < 2:
            ajudas += 1
            ajuda = gera_ajuda(questao)
            print(ajuda)
            print('dinheiro atual:',dinheiro_atual)
        elif resposta == lista_respostas_2[0] and ajudas >= 2:
            print("vc não tem mais ajudas")
            print('dinheiro atual:',dinheiro_atual)

        elif resposta in lista_respostas:
            if resposta == questao["correta"]:
                ...
                print('dinheiro atual:',dinheiro_atual)
        if resposta not in lista_respostas and resposta not in lista_respostas_2:
